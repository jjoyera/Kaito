...
- [x] 4.2 Historical task: the former direct-jump navigator implementation is obsolete and superseded by the current linear Back/Continue flow with non-interactive progress; it must not be treated as active behavior.
- [x] 4.3 Build `completion-view.tsx` for the post-completion confirmation state.
- [x] 4.4 Build `onboarding-wizard.tsx`: owns the accumulated snapshot and step-index state, wires the load/save/complete use-cases, enforces per-step completion before advancing, and drives navigator status from `diagnostic-mapping.ts`. The former array-normalization detail is superseded by the current reduced clean-state contract.
...
- [x] 6.1 Add Playwright coverage for resume-from-draft hydration, save-on-advance persistence, per-step completion gating, conditional field clearing, completion success, and completion demotion with diagnostics. The former direct-jump wording and historical count are superseded by the current linear-navigation verification.
