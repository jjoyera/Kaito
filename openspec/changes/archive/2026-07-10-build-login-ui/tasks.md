...
- [x] Update `apps/web/app/styles.css` with Kaito login tokens and page styles: warm sand background, off-white card, forest/deep green primary, restrained gold/orange accents, and earth-red errors.
- [x] Use `.context/Kaito AI Running Coach.zip` / `Kaito.dc.html` as the primary visual reference for background depth, motion, card styling, compact brand header, mountain/sun language, and golden trail accent.
- [x] Do not copy the mockup password-recovery affordance; password reset remains out of scope.
- [x] Keep background and motion secondary to readable form focus; avoid generic blue SaaS, futuristic AI clichés, medical styling, and aggressive gym branding.
...
- [x] Ensure `aria-invalid`, `aria-describedby`, and semantic alert/live-region behavior for field-level and form-level feedback in `apps/web/features/auth/login-form.tsx`.
- [x] Add or extend Playwright checks in `apps/web/e2e/login.spec.ts` for accessible names, focus order, feedback exposure, and reduced-motion behavior.
- [x] Add `@media (prefers-reduced-motion: reduce)` handling in `apps/web/app/styles.css` so decorative animation is disabled without changing essential behavior.
- [x] Verify mobile and desktop layouts avoid horizontal overflow and keep touch targets comfortable.
...
- [x] Review `apps/web/features/auth/*` for duplication, provider leakage, raw error exposure, and oversized components.
- [x] Keep route composition in `apps/web/app/(auth)/login/page.tsx` thin; move form logic only to feature files.
- [x] Keep copy brief and coaching-oriented; do not turn `/login` into a marketing landing page.
- [x] Confirm no backend/API files are touched unless a new discovery makes that unavoidable and separately approved.
...
- [x] Run expected frontend validation commands after implementation: `pnpm lint:web`, `pnpm build:web`, and `pnpm test:web-e2e`.
- [x] If package scripts were added for unit tests, run the corresponding web/root test command and document it in the apply result.
- [x] Update `openspec/changes/build-login-ui/artifacts.md` during apply/verify with implementation and verification status.
- [x] During sync, review `README.md`; update it only if this login UI changes stable capabilities, setup/environment variables, architecture/runtime behavior, or developer commands.
